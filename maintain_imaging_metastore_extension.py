# Fabric Notebook: Maintain Imaging Metastore Extension
...
RUN_OPTIMIZE = parse_bool_param("RUN_OPTIMIZE", False)
RUN_VACUUM = parse_bool_param("RUN_VACUUM", False)
CONFIRM_MAINTENANCE = str(get_param("CONFIRM_MAINTENANCE", "")).strip()
VACUUM_RETAIN_HOURS = int(get_param("VACUUM_RETAIN_HOURS", "168"))
RUN_TABLE = "ImagingMetastoreExtensionMaintenanceRun"
RUN_SCHEMA = T.StructType([
    T.StructField("maintenanceRunId", T.StringType(), False),
...
if (RUN_OPTIMIZE or RUN_VACUUM) and (DRY_RUN_ONLY or CONFIRM_MAINTENANCE != "MAINTAIN_IMAGING_METASTORE_EXTENSION"):
    raise ValueError("OPTIMIZE/VACUUM require DRY_RUN_ONLY=false and CONFIRM_MAINTENANCE='MAINTAIN_IMAGING_METASTORE_EXTENSION'.")
rows = spark.read.format("delta").load(TARGET_PATH).count() if DeltaTable.isDeltaTable(spark, TARGET_PATH) else 0
messages = [f"targetRows={rows}"]
if RUN_OPTIMIZE:
    spark.sql(f"OPTIMIZE delta.`{TARGET_PATH}`")
    messages.append("OPTIMIZE completed")
if RUN_VACUUM:
    spark.sql(f"VACUUM delta.`{TARGET_PATH}` RETAIN {VACUUM_RETAIN_HOURS} HOURS")
    messages.append(f"VACUUM completed retainHours={VACUUM_RETAIN_HOURS}")
status = "DryRunSucceeded" if DRY_RUN_ONLY else "Succeeded"
row={"maintenanceRunId":run_id,"startedAt":current_utc_naive(),"completedAt":current_utc_naive(),"status":status,"dryRunOnly":DRY_RUN_ONLY,"targetRows":rows,"message":"; ".join(messages)}
