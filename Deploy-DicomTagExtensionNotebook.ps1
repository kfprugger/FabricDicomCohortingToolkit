...
$sourcePath = if ([System.IO.Path]::IsPathRooted($SourceFile)) { $SourceFile } else { Join-Path $PSScriptRoot $SourceFile }
...
foreach ($line in $allSourceLines) {
    ...
    if ($inParameterBlock) {
        if ($line -eq "" -and $parameterLines.Count -gt 0) {
            $inParameterBlock = $false
            continue
        }
        $parameterLines += $line
...
# DICOM Tag Extension Notebook

This notebook is part of the DICOM Tag Extension operations set.
'@
$parametersMarkdownLines = ConvertTo-NotebookSourceLines -Source @'
## Parameters

The next cell is a Fabric parameter cell. Parameters may be overridden by `RunNotebook` execution data.
'@
$executionMarkdownLines = ConvertTo-NotebookSourceLines -Source @'
## Execution

The main code cell contains the notebook implementation from the deployed source file.
'@
...
if ($RunAfterDeploy) {
    ...
    $jobId = $Matches[1]
    Wait-FabricNotebookJob -WorkspaceId $WorkspaceId -NotebookId $notebookId -JobId $jobId -Headers $script:FabricHeaders -TimeoutSeconds $JobTimeoutSeconds | Out-Null
