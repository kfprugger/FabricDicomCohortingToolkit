...
|---|---|---|
| Condition | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(fc.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(fc.subject_string, '$.idOrig'), '')) = p.idOrig |
| Encounter | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(enc.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(enc.subject_string, '$.idOrig'), '')) = p.idOrig |
| MedicationRequest | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(mr.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(mr.subject_string, '$.idOrig'), '')) = p.idOrig |
| Observation | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(obs.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(obs.subject_string, '$.idOrig'), '')) = p.idOrig |
| DiagnosticReport | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(dr.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(dr.subject_string, '$.idOrig'), '')) = p.idOrig |
| Procedure | subject_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(pr.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(pr.subject_string, '$.idOrig'), '')) = p.idOrig |
| AllergyIntolerance | patient_string | COALESCE(REPLACE(NULLIF(JSON_VALUE(ai.patient_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(ai.patient_string, '$.idOrig'), '')) = p.idOrig |
| ImagingStudy | subject_string | COALESCE(NULLIF(JSON_VALUE(ims.subject_string, '$.identifier.value'), ''), REPLACE(NULLIF(JSON_VALUE(ims.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(ims.subject_string, '$.idOrig'), '')) = p.idOrig |

IMPORTANT: Most clinical tables should prefer `$.msftSourceReference` and fall back to `$.idOrig`; do not use `$.identifier.value` for those joins. ImagingStudy is the exception and should prefer `$.identifier.value`.
AllergyIntolerance uses patient_string (not subject_string).
...
ALWAYS add this filter to avoid truncated JSON: AND ISJSON(ims.series_string) = 1
  ...
  JOIN dbo.Patient p ON COALESCE(NULLIF(JSON_VALUE(ims.subject_string, '$.identifier.value'), ''), REPLACE(NULLIF(JSON_VALUE(ims.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(ims.subject_string, '$.idOrig'), '')) = p.idOrig
...
All medications are in dbo.MedicationRequest (dbo.MedicationStatement has 0 rows). Example:
  ...
  JOIN dbo.Patient p ON COALESCE(REPLACE(NULLIF(JSON_VALUE(mr.subject_string, '$.msftSourceReference'), ''), 'Patient/', ''), NULLIF(JSON_VALUE(mr.subject_string, '$.idOrig'), '')) = p.idOrig
  WHERE mr.medicationCodeableConcept_string LIKE '%lisinopril%'
...
| Condition | code_string ($.coding[0].display), clinicalStatus_string, severity_string, stage_string, onsetDateTime, recordedDate | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| ImagingStudy | started, numberOfSeries, numberOfInstances, description, series_string ($.modality.code) | subject_string: prefer `$.identifier.value`, fallback stripped `$.msftSourceReference`, fallback `$.idOrig` |
| DiagnosticReport | conclusion, code_string, effectiveDateTime, imagingStudy_string | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| AllergyIntolerance | code_string, criticality, type, reaction_string, clinicalStatus_string | patient_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| MedicationRequest | medicationCodeableConcept_string ($.coding[0].display), status, authoredOn | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| Observation | category_string, code_string, valueQuantity_string, valueString | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| Encounter | class_string ($.code), type_string, period_string ($.start, $.end) | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
| Procedure | code_string, performedDateTime, bodySite_string | subject_string: prefer `$.msftSourceReference` stripped of `Patient/`, fallback `$.idOrig` |
